...
class FibreDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        resized_Mask = mask.resize((300, 300))
        ...
        contours, _ = cv2.findContours(resized_Mask, cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)

        num_objs = len(contours)
        # Create an empty numpy array to store the masks
        mask_shape = resized_Mask.shape
        ...
        for i, contour in enumerate(contours):
            ...
            contour_mask = np.zeros_like(resized_Mask, dtype=np.uint8)
    ...
    #         masks.append(contour_mask)       
        ...
        #     #sample = self.transforms(sample)
            resized_IMG = self.transforms(resized_IMG)
        ...
        data = [(resized_IMG, target)]  # Create a list with a single tuple
